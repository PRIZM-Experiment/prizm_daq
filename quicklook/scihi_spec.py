...
def get_data():	
    ep0 = np.array(struct.unpack('>1024Q', fpga.read('even_pol0',1024*8,0)))
    #print ep0
    op0 = np.array(struct.unpack('>1024Q', fpga.read('odd_pol0',1024*8,0)))
    #print op0
    ep1 = np.array(struct.unpack('>1024Q', fpga.read('even_pol1',1024*8,0))) 	
    op1 = np.array(struct.unpack('>1024Q', fpga.read('odd_pol1',1024*8,0)))
    even_real = np.array(struct.unpack('>1024q', fpga.read('even_real',1024*8,0)))
    even_imaginary = np.array(struct.unpack('>1024q', fpga.read('even_imaginary',1024*8,0)))
    odd_real = np.array(struct.unpack('>1024q', fpga.read('odd_real',1024*8,0)))
    odd_imaginary = np.array(struct.unpack('>1024q', fpga.read('odd_imaginary',1024*8,0)))
...
if __name__ == '__main__':
    ...
    p.add_option('-i', '--ip', dest='ip', type='str',default='146.230.231.73',
